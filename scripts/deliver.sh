...
now=`date +'%m-%d-%Y--%H-%M'`
archive_name=philippejoulot-$now.tgz
...
cd _site/ && tar -zcf "../releases/$archive_name" . && cd ..
...
ssh $DELIVERY_USER@$DELIVERY_SERVER "rm -Rf $DELIVERY_DIR*;tar -zxf /home/$DELIVERY_USER/$archive_name -C $DELIVERY_DIR;rm -f /home/$DELIVERY_USER/$archive_name; chmod -R 774 $DELIVERY_DIR; chown -R $DELIVERY_USER:www-data $DELIVERY_DIR"
